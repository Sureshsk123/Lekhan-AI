...
describe('Global Search Engine Integration Tests', () => {
  test('GET /api/search — performs global search across all categories', async () => {
    const res = await request(app)
      .get('/api/search?q=Tamil&category=all')
  ...
  test('GET /api/search — searches specific category (lessons)', async () => {
    const res = await request(app)
      .get('/api/search?q=Alphabet&category=lessons')
      .set('Authorization', `Bearer ${token}`);
